...
1. 下载最新版本的 `main.js` 和 `manifest.json` 文件
2. 在你的 Obsidian 库中找到 `.obsidian/plugins` 目录
3. 创建一个新文件夹 `dida-sync-v2`
4. 将下载的 `main.js` 和 `manifest.json` 放入该文件夹
5. 重启 Obsidian
6. 打开 设置 → 第三方插件
...
### Q: 为什么生日提醒项目没有显示？
A: 生日提醒是一个普通项目，不是特殊项目。只要你的滴答清单中有生日提醒项目，并且有任务，它就会自动显示。如果没有显示，请检查：
1. 你的滴答清单中是否确实有这个项目
2. 该项目中是否有任务
3. 尝试点击刷新按钮

### Q: 收集箱和其他项目有什么区别？
A: 收集箱（Inbox）是滴答清单的系统功能，是唯一的特殊项目。它总是存在的，用于快速收集任务。其他所有项目（包括生日提醒）都是普通的用户项目。
